                ...
                else if(loadstate == 1)
                {
                    if(saveType == Save_Type.Components){
                        ...
                        //!customSave
                            ...
                            if (CheckLoadState(loadstate, tempSave))
                            {
                            ...
                            {
                                Debug.Log("<color=yellow>[SaveGameHandler]</color> Could not load file: " + tempSave);
                                Prefs.Game_LoadState(0);
                            }
                ...
                if (Directory.Exists(path))
                {
                    ...
                    if(saveType == Save_Type.Components){
                        ...
                        foreach(FileInfo fi in di.GetFiles()) {
                            ...
                            for(int i = 0; i < HFPS_CompSave.instance.saveData.levelData.levels.Length; ++i ) {
                        ...
                        if(HFPS_CompSave.instance != null){
                            ...
                            HFPS_CompSave.instance.Level_Add(SceneManager.GetActiveScene().name, newSave);
                    ...
                    //!useSaveLimit
                        ...
                        //count < maxSaves
                            ...
                            if(Prefs.Exist("LastSave")){
                                ...
                                File.Delete(Path.Combine(path, lastSave));
                        ...
                        if(tempSaves.Count > 0){
                            ...
                            for(int i = 0; i < tempSaves.Count; ++i ) {
                                ...
                                if(tempSaves[i] == newSave){
                                    ...
                                    File.Delete(Path.Combine(path, newSave));

                                    //Debug.Log("Save File Deleted For New Save = " + newSave);
